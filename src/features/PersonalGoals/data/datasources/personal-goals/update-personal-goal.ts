...
export interface IUpdatePersonalGoalParams {
...
class UpdatePersonalGoal {
  private endpoint: string;

  private params: IUpdatePersonalGoalParams;

  constructor(params: IUpdatePersonalGoalParams) {
    this.endpoint = 'api/metas-pessoais';
    this.params = params;
  }

  exec() {
    api.put<ApiData>(`${this.endpoint}/${this.params.personalGoal.id}`, this.convertToSend(this.params.personalGoal))
  ...
  convertToSend(goal: IPersonalGoal) {
    return {
      ...goal,
      codigo: goal.id,
